...
using Gp_Api.Services; // 確保引用了包含 ProjectInternalView 的命名空間
using System;
using System.Linq;
...
namespace Gp_Api.Controllers
{
    ...
    [ApiController]
    public class ProjectInternalController : ControllerBase
    {
        private readonly IProjectInternalService _service;
        private readonly IHubContext<ChatHub> _hubContext;

        public ProjectInternalController(IProjectInternalService service, IHubContext<ChatHub> hubContext)
        {
            _service = service;
        ...
        [HttpPost]
        public IActionResult Post(ProjectInternalView data)
        {
            try
            {
                var roleIdClaim = User.Claims.FirstOrDefault(t => t.Type == "role_id");
                ...
                // 透過 SignalR 通知前端更新
                _hubContext.Clients.All.SendAsync("ProjectInternal", "新增");
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                ...
                if (res == "OK")
                {
                    _hubContext.Clients.All.SendAsync("ProjectInternal", "刪除");
        ...
        [HttpPatch("{id}")]
        public IActionResult Edit(int id, ProjectInternalView data)
        {
            try
            {
                ...
                if (res == "OK")
                {
                    _hubContext.Clients.All.SendAsync("ProjectInternal", "更新");
